...
def suspend_new_duplicates_by_lemma(
    ...
    if missing_note_ids:
        for info in client.get_notes_info(missing_note_ids):
            note_cards[info["noteId"]] = info.get("cards", [])

    all_card_ids = sorted({cid for cards in note_cards.values() for cid in cards})
    if not all_card_ids:
        return {"duplicate_notes": 0, "cards_suspended": 0, "dry_run": int(dry_run)}

    new_cards_by_note: dict[int, list[dict[str, Any]]] = {}
    for card in client.get_cards_info(all_card_ids):
        if card.get("queue") != 0:
            continue
        nid = card.get("note")
        if nid is None:
            continue
        new_cards_by_note.setdefault(nid, []).append(card)

    new_note_ids = set(new_cards_by_note)
    if not new_note_ids:
        return {"duplicate_notes": 0, "cards_suspended": 0, "dry_run": int(dry_run)}

    new_notes = [note_by_id[nid] for nid in new_note_ids if nid in note_by_id]
    ...
    def _best_new_card_key(cards: list[dict[str, Any]]) -> tuple[int, int]:
        best_due = 10**12
        best_id = 10**18
        for card in cards:
            due = card.get("due")
            if isinstance(due, int) and due < best_due:
                best_due = due
                best_id = card.get("cardId") or card.get("id") or best_id
            elif isinstance(due, int) and due == best_due:
                cid = card.get("cardId") or card.get("id") or best_id
                if cid < best_id:
                    best_id = cid
        return best_due, best_id

    cards_to_suspend: set[int] = set()
    notes_to_tag: set[int] = set()

    for _, nids in grouped.items():
        if len(nids) < 2:
            continue
        keep_nid = min(nids, key=lambda nid: _best_new_card_key(new_cards_by_note.get(nid, [])))
        for nid in nids:
            if nid == keep_nid:
                continue
            notes_to_tag.add(nid)
            for card in new_cards_by_note.get(nid, []):
                cid = card.get("cardId") or card.get("id")
                if cid is not None:
                    cards_to_suspend.add(cid)

    card_ids_sorted = sorted(cards_to_suspend)
    note_ids_sorted = sorted(notes_to_tag)
    ...
    if not dry_run:
        ...
        done_notes = 0
        for batch in client.chunked(note_ids_sorted):
            client.invoke("addTags", notes=batch, tags=tag)
            done_notes += len(batch)
            _print_progress("Tag duplicates", done_notes, len(note_ids_sorted))
